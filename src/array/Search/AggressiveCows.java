...
** Approach:
** Ref: https://www.youtube.com/watch?v=YTTdLgyqOLY
...
** Time Complexity: O(nlogn), as we need to sort first
  ...
  **/
    public static boolean isPossibleSol(int stalls[], int k, int mid) {
        ...
        for (int i = 1; i < stalls.length; i++) {
            if (Math.abs(stalls[i] - lastPos) >= mid) {
                cowCount++;
                lastPos = stalls[i];
                if (cowCount == k) {
                    return true;
    ...
    public static int aggressiveCows(int stalls[], int k) {
        Arrays.sort(stalls);
        int l = 0;
        int h = Arrays.stream(stalls).max().getAsInt();
        int ans = -1;
        while (l <= h) {
            int mid = l + (h - l) / 2;
